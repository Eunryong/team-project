...
def main():
    ...
    structures = [(xy, struct) for xy, struct in struct_dict.items() if struct in target_types]
    if any(struct == 'MyHome' for xy, struct in structures):
        structures.sort(key=lambda x: 0 if x[1] == 'MyHome' else 1)
    else:
        structures = [((1, 1), 'Start')] + structures

    positions = [xy for xy, struct in structures]
    labels = [struct for xy, struct in structures]
    n = len(positions)
    ...
    plt.tight_layout()
    plt.savefig('map_final_tsp.png', dpi=150)
    plt.close()
